 ...
 *
 *   npm run bp:add -- --parent BP-10.10 --name "Forecast Reconciliation"
 *
 * Picks the next sparse sibling id automatically (parent.id + .<n>0).
 */
...
if (!parentId || !name) {
  console.error(
    "Usage: bp:add --parent <BP-id> --name '<Display Name>' [--description '...'] [--realizes BC-id,BC-id,...]"
  );
  process.exit(2);
}
...
if (!fileEntry) {
  console.error(`Parent ${parentId} not found in any BP1 file.`);
  process.exit(1);
}
if (!existsSync(join(PROCESSES_DIR, fileEntry.name))) {
  console.error(`File ${fileEntry.name} missing on disk under catalogue/processes/.`);
  process.exit(1);
}
...
const newId = nextChildId(parentNode, BP_PREFIX);
const realizes = args.realizes
  ? args.realizes.split(",").map((s) => s.trim()).filter(Boolean)
  : undefined;
const newNode: CliRawCapability & {
  ...
  ...(realizes && realizes.length > 0 ? { realizes_capability_ids: realizes } : {}),
  children: [],
};
...
const visit = (yamlNode: YAML.YAMLMap, raw: CliRawCapability): boolean => {
  ...
  for (let i = 0; i < childrenSeq.items.length; i++) {
    ...
    const childRaw = (raw.children ?? []).find((c) => c.id === childYaml.get("id"));
...
saveTree(fileEntry.name, doc, BP_PREFIX);
console.log(`✔ Added BP ${newId} '${name}' under ${parentId} in processes/${fileEntry.name}`);
