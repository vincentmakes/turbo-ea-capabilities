...
if (!id || !reason) {
  console.error("Usage: bp:deprecate --id <BP-id> --reason '<text>' [--successor <BP-id>]");
  process.exit(2);
}
...
saveTree(fileEntry.name, fileEntry.doc, BP_PREFIX);
console.log(`✔ Deprecated BP ${id} in processes/${fileEntry.name}`);
